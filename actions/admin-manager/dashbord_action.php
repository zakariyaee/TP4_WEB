...
if (isset($_SESSION['user_role']) &&  $_SESSION['user_role'] == 'responsable') {
    ...
    ");
    $stmt->execute([':responsable_id' => $_SESSION['user_email']]);
    ...
    $requete = "Select count(*) from terrain where disponibilite='disponible' AND id_responsable=:responsable_id";
    $req = $pdo->prepare($requete);
    $req->bindParam(':responsable_id', $_SESSION['user_email']);
    $_SESSION['total_terrains'] = $req->fetchColumn();
    $requete = "Select count(*) from reservation where statut='confirmee' AND id_terrain IN (SELECT id_terrain FROM terrain WHERE id_responsable=:responsable_id)";
    $req = $pdo->prepare($requete);
    $req->bindParam(':responsable_id', $_SESSION['user_email']);
    ...
    ";
        ...
        foreach ($ids_terrain as $id_terrain) {
            $requete = "SELECT prix_heure FROM terrain WHERE id_terrain = :id_terrain AND id_responsable=:responsable_id";
            $req2 = $pdo->prepare($requete);
            $req2->bindParam(':id_terrain', $id_terrain);
            $req2->bindParam(':responsable_id', $_SESSION['user_email']);
            $req2->execute();
    ...
    // diagrame circulair 
    $resq = "Select count(*) as total_minifoot from terrain where taille='70x40m' AND id_responsable=:responsable_id";
    $req = $pdo->prepare($resq);
    $req->bindParam(':responsable_id', $_SESSION['user_email']);
    $req->execute();
    $_SESSION['total_minifoot'] = $req->fetchColumn();
    $resq = "Select count(*) as total_moyenne from terrain where taille='90x50m' AND id_responsable=:responsable_id";
    $req = $pdo->prepare($resq);
    $req->bindParam(':responsable_id', $_SESSION['user_email']);
    $req->execute();
    $_SESSION['total_moyenne'] = $req->fetchColumn();
    $resq = "Select count(*) as total_Grand from terrain where taille='105x68m' AND id_responsable=:responsable_id";
    $req = $pdo->prepare($resq);
    $req->bindParam(':responsable_id', $_SESSION['user_email']);
    $req->execute();
...
//-------------------------------------------------
//----------------------------------------------------
// partie admin traitement des données du dashboard           
else {
    $_SESSION['revenue_total'] = 0;
    $revenue_total = 0;

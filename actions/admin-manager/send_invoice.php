...
try {
    ...
    // Générer le HTML (même code que generate_invoice.php)
    $html = '
    <!DOCTYPE html>
    <html>
    <head>
    ...
    $mail->addAddress($facture['email_client'], $facture['prenom_client'] . ' ' . $facture['nom_client']);
    
    // Contenu de l'email
    $mail->isHTML(true);
    $mail->Subject = 'Votre facture ' . $facture['numero_facture'];
    $mail->Body = '
    <html>
    <head>
        <style>
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p>Bonjour ' . htmlspecialchars($facture['prenom_client']) . ',</p>
                
                <p>Nous vous remercions pour votre réservation.</p>
                
                <p><strong>Détails de votre réservation :</strong></p>
                <ul>
                    <li><strong>Numéro de facture :</strong> ' . htmlspecialchars($facture['numero_facture']) . '</li>
                    <li><strong>Terrain :</strong> ' . htmlspecialchars($facture['nom_terrain']) . '</li>
                    <li><strong>Date :</strong> ' . date('d/m/Y', strtotime($facture['date_reservation'])) . '</li>
                    <li><strong>Horaire :</strong> ' . substr($facture['heure_debut'], 0, 5) . ' - ' . substr($facture['heure_fin'], 0, 5) . '</li>
                    <li><strong>Montant total :</strong> ' . number_format($facture['montant_total'], 2, ',', ' ') . ' DH</li>
                </ul>
            ...
            <div class="footer">
                <p><strong>Gestion Terrains</strong><br>
                123 Avenue Mohammed V, Tétouan<br>
                Tél: +212 539 123 456<br>
                Email: contact@tour.com</p>
            </div>
        </div>
    </body>
    </html>';
    ...
    echo 'Facture envoyée avec succès à ' . $facture['email_client'];
    
} catch (Exception $e) {
    http_response_code(500);
    echo 'Erreur lors de l\'envoi de la facture : ' . $e->getMessage();
}
?>